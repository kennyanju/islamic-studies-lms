...
class EmailService {
  ...
  initTransporter() {
    ...
    if (host && user && pass) {
      ...
      this.transporter = null;
      console.log(
        '📧 [Email] Using Simulated/Preview Mailer (Set SMTP_HOST, SMTP_USER, SMTP_PASS for live delivery)'
      );
    }
  }
  ...
  async sendMail({ to, subject, html, text, type = 'notification' }) {
    const fromAddress =
      process.env.SMTP_FROM || 'Islamic Studies LMS <no-reply@islamicstudies.local>';
    const mailRecord = {
    ...
    if (process.env.RESEND_API_KEY) {
      try {
        const fromEmail =
          ...
          'Islamic Studies LMS <onboarding@resend.dev>';
        ...
        if (res.ok) {
          console.log(`✉️ [Resend Email Sent] ${type} to ${to} (${subject})`);
          return { success: true, mailId: mailRecord.id, live: true, data };
        } else {
          console.error(`❌ [Resend Error] Failed delivering to ${to}:`, data);
          return {
            success: false,
        ...
        console.error(`❌ [Resend Exception] Failed delivering to ${to}:`, resendErr.message);
      }
    }
  ...
  /**
   * Quiz Completion & Certificate Notification Email to Parent
   */
  async sendQuizCompletionEmail({
    ...
    const statusText = passed ? 'Passed with Distinction' : 'Quiz Completed';
    const subject = `${statusEmoji} ${childName || 'Your learner'} completed Module ${moduleId}: ${moduleTitle || 'Islamic Studies'}`;
    const text = `Assalamu Alaikum ${parentName || 'Parent'},\n\nYour learner ${childName || 'Student'} just completed the examination for Module ${moduleId} (${moduleTitle || ''}).\n\nResult: ${score}/${total} (${percentage}%) - ${statusText}.\n\nLog in to review their detailed answers and view the Certificate of Completion:\n${appUrl}\n\nIslamic Studies Team`;
    ...
    const html = `
      ...
      <head>
        ...
        <style>
          body { font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; background-color: #0f172a; color: #f8fafc; margin: 0; padding: 20px; }
          .container { max-width: 600px; margin: 0 auto; background: #1e293b; border-radius: 12px; border: 1px solid #334155; overflow: hidden; }
          .header { background: linear-gradient(135deg, ${passed ? '#064e3b' : '#1e3a8a'}, #0f172a); padding: 28px; text-align: center; }
          .header h1 { color: #fbbf24; margin: 0 0 6px 0; font-size: 24px; }
          .header p { color: #94a3b8; margin: 0; font-size: 14px; }
          .content { padding: 28px; line-height: 1.6; }
          .score-card { background: #0f172a; border: 1px solid ${passed ? '#059669' : '#3b82f6'}; border-radius: 10px; padding: 20px; text-align: center; margin: 20px 0; }
          .score-number { font-size: 36px; font-weight: 800; color: ${passed ? '#34d399' : '#60a5fa'}; margin: 4px 0; }
          .btn { display: inline-block; background: #059669; color: #ffffff !important; padding: 12px 24px; text-decoration: none; border-radius: 8px; font-weight: 600; margin: 20px 0; }
          .footer { background: #0f172a; padding: 16px 28px; text-align: center; font-size: 12px; color: #64748b; border-top: 1px solid #334155; }
        </style>
      </head>
      <body>
        <div class="container">
          <div class="header">
            ...
            <p>Module ${moduleId}: ${moduleTitle || 'Islamic Studies'}</p>
          </div>
          <div class="content">
            <h2>Assalamu Alaikum, ${parentName || 'Parent'}!</h2>
            <p><strong>${childName || 'Your learner'}</strong> just completed the quiz assessment for <strong>Module ${moduleId}: ${moduleTitle || ''}</strong>.</p>
            
            <div class="score-card">
              <div>Score Achieved</div>
              <div class="score-number">${percentage}%</div>
              <div style="color: #94a3b8;">${score} out of ${total} correct answers (${passed ? 'Exam Passed ✅' : 'Review Recommended 📖'})</div>
            </div>

            <p>You can review their question breakdown, reflection answers, and verified completion certificate inside your family portal.</p>

            <center>
              <a href="${appUrl}" class="btn">View Learner Progress</a>
            </center>
          </div>
          <div class="footer">
            <p>&copy; ${new Date().getFullYear()} Islamic Studies Family LMS. 100% Private, Secure & Ad-Free.</p>
    ...
    return this.sendMail({ to: parentEmail, subject, text, html, type: 'quiz_completion' });
  }

  /**
   * Compatibility wrapper matching Cloudflare Workers Edge Email signature
   */
  async sendEdgeEmail({ to, subject, html, text, type = 'notification' }) {
    return this.sendMail({ to, subject, html, text, type });
  }
}
