...
class DatabaseAdapter {
  ...
  async getChildById(childId) {
    ...
    return this.memoryData.children.find(c => 
      ...
      (c.id && (c.id.toLowerCase().includes(clean) || clean.includes(c.id.toLowerCase()))) ||
      (c.name && (c.name.trim().toLowerCase().includes(clean) || clean.includes(c.name.trim().toLowerCase())))
    ) || null;
  }
